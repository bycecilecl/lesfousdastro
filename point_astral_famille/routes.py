...
from point_astral_famille.orchestration import generer_point_astral_blocs
from point_astral_famille.rendu import transformer_en_sections
from utils.genre import get_user_prefs
from point_astral_famille.selection_donnees import construire_selection_point_astral
...
from point_astral_famille.configurations_astrologiques import (
    analyser_conjonctions,
    analyser_stelliums,
    analyser_configurations_majeures,
...
def _extraire_positions_configurations(data_theme):
    """
    Extrait les positions planétaires dans le format attendu par
    utils.configurations_astrologiques.
    ...
    La fonction privilégie les structures riches contenant
    signe, degré et maison.
    """
    if not isinstance(data_theme, dict):
        return {}

    sources_possibles = [
        data_theme.get("planetes"),
        data_theme.get("positions_planetes"),
        data_theme.get("planets"),
        _dig(data_theme, "positions", "planetes"),
        _dig(data_theme, "astro", "planetes"),
        data_theme.get("planetes_deg"),
        data_theme.get("planets_deg"),
        data_theme.get("positions_deg"),
    ]
    ...
    for source in sources_possibles:
        ...
        for planete, valeur in source.items():
            ...
            degre = valeur.get("degre")

            if degre is None:
                degre = valeur.get("degree")

            maison = valeur.get("maison")

            if maison is None:
                maison = valeur.get("house")

            longitude = valeur.get("longitude")

            if longitude is None:
                longitude = valeur.get("degre_absolu")

            if longitude is None:
                longitude = valeur.get("position_absolue")

            positions_source[str(planete).strip()] = {
                "signe": signe,
                "degre": degre,
                "maison": maison,
                "longitude": longitude,
        ...
        # On privilégie la première source contenant de vrais signes.
        contient_signes = any(
            position.get("signe")
            for position in positions_source.values()
        )

        if contient_signes:
            return positions_source
...
# ---------- Blueprint ----------
point_astral_famille_bp = Blueprint(
    "point_astral_famille",
    __name__,
    url_prefix="/point-astral-famille"
)

def generer_point_astral_famille_pdf_s3(infos, envoyer_email=False):
    """
    ...
    if is_analysis_sandbox():
        ...
        return {
            "product_id": "point_astral_famille",
            "label": "Point Astral – Racines familiales",
            "pdf_url": "https://sandbox.lesfousdastro.fr/point-astral-famille-test",
            "pdf_path": None,
            "status": "sandbox",
        }
    ...
    missing = [f for f in required_fields if not infos.get(f)]

    if missing:
        raise ValueError(f"Champs manquants Flash Astral: {missing}")
    ...
    try:
        ...
        current_app.logger.warning(f"Logo non chargé: {e}")
    ...
    nom_fichier = f"Point_Astral_Famille_{nom}_{timestamp}"
    ...
    pdf_final_url = upload_client_pdf(
        pdf_path,
        key_prefix="point_astral_famille",
        download_filename=f"{nom_fichier}.pdf",
    )
...
@point_astral_famille_bp.route("/__ping", methods=["GET"])
def ping_blocs():
    return "ok blocs"
...
def point_astral_famille_complet():
    ...
    try:
        ...
        print("✅ Section Aspects?", "### Aspects (≤ 5° d'orbe)" in placements_str)
                ...
                # 3 bis) Détection des configurations astrologiques
        aspects_theme = data_theme.get("aspects") or []

        positions_configurations = (
            _extraire_positions_configurations(data_theme)
        )

        print(
            "🔗 Configurations → aspects disponibles :",
            len(aspects_theme),
        )
        print(
            "🔗 Configurations → positions disponibles :",
            list(positions_configurations.keys()),
        )

        print(
            "🔍 Exemple position Jupiter :",
            positions_configurations.get("Jupiter"),
        )

        try:
            configurations_majeures = (
                analyser_configurations_majeures(
                    aspects=aspects_theme,
                    positions=positions_configurations,
                )
            )
            ...
            t_carres = [
                configuration
                for configuration in configurations_majeures
                if configuration.get("type") == "t_carre"
            ]

            print(
                f"✅ Configurations détectées : "
                f"{len(conjonctions)} conjonction(s), "
                f"{len(stelliums)} stellium(s), "
                f"{len(t_carres)} T-carré(s)"
            )

        except Exception as e:
            print(
                f"⚠️ Erreur détection configurations : {e}"
            )
            ...
            warnings_list.append(
                "Configurations astrologiques indisponibles."
            )

        data_theme["conjonctions"] = conjonctions
        data_theme["stelliums"] = stelliums
        data_theme["t_carres"] = t_carres
        data_theme["configurations_majeures"] = configurations_majeures
        configurations_majeures_str = formater_configurations_majeures(
            configurations_majeures
        )

        print("🪐 Configurations majeures :")
        print(configurations_majeures_str)
        ...
        if not placements_str or len(placements_str) < 40:
            return render_template(
                ...
                details=f"placements_str: {len(placements_str)} chars"
            )
        ...
        return render_template(
            'point_astral_famille_resultat.html',
            nom=infos["nom"],
...
@point_astral_famille_bp.route("/apercu_point_astral/<nom_fichier>", methods=["GET"])
...
@point_astral_famille_bp.route("/telecharger_point_astral/<nom_fichier>", methods=["GET"])
    ...
    <div class="header" style="text-align: center; margin-bottom: 20px;">
        ...
        <div class="disclaimer" style="background:#f8f9fa;border:1px solid #dee2e6;
            ...
            <p style="margin:0 0 8px 0;">
                <strong>⚠️ Le Point Astral est une lecture automatisée express (≈6 pages).</strong><br>

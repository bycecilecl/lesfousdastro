...
from point_astral_famille.calculs_astrologiques import detecter_amas_et_zones_chargees
import re as regex
import math
import unicodedata
...
def aspects_maitre_ascendant(maitre_asc: str | dict, aspects: list, second_maitre: str | None = None, max_orbe: float = 5.0) -> list:
    """
    ...
    maitres = {maitre_nom.capitalize()}

    if second_maitre:
        maitres.add(second_maitre.capitalize())
    ...
    for a in aspects or []:
        ...
        if asp_label.startswith("conjonction") and orbe <= max_orbe:
            if p1 in maitres or p2 in maitres:
                b = dict(a)
...
def detecter_amas_personnels(occ: Dict[str, Any], min_perso: int = 3) -> Dict[str, list]:
    ...
    Utilise désormais la détection centrale des amas planétaires.
    Retourne l'ancien format attendu :
      {
        "signes": [...],
        "maisons": [...]
      }

    Le paramètre min_perso est conservé pour ne pas casser les anciens appels,
    mais la règle commune est maintenant gérée par
    detecter_amas_et_zones_chargees().
    """
    resultat = detecter_amas_et_zones_chargees(occ)

    return {
        "signes": resultat.get("amas_signes", []),
        "maisons": resultat.get("amas_maisons", []),
    }
...
def _get_points_forts_str(data: Dict[str, Any]) -> str:
    ...
    # 6.5) ➕ Injecter les AXES INTERCEPTÉS dans "Informations complémentaires"
    try:
        inter = _extraire_axes_interceptes(data)  # {'signes': [...], 'maisons_par_signe': {...}}
        inter_lines = []
        if inter.get("signes"):
            inter_lines.append("Signes interceptés : " + ", ".join(inter["signes"]))
        for signe, maison in (inter.get("maisons_par_signe") or {}).items():
            inter_lines.append(f"{signe} intercepté en {maison}")

        # On les met au DÉBUT des infos compl., pour garantir qu'ils soient vus
        if inter_lines:
            # Evite les doublons exacts si déjà ajoutés ailleurs
            ic_lower = {s.lower() for s in infos_compl}
            for ln in reversed(inter_lines):
                if ln.lower() not in ic_lower:
                    infos_compl.insert(0, ln)
    except Exception:
        pass
...
def construire_selection_analyse_gratuite(data: Dict[str, Any], max_orbe: float = 5.0) -> str:
    ...
    # 👇 Axes interceptés EN TÊTE (pas de recalcul, on lit ce qui existe déjà)
    axes = _extraire_axes_interceptes(data)
    signes_int = axes["signes"]
    maisons_int = axes["maisons_par_signe"]

    if signes_int or maisons_int:
        blocs.append("### Axes interceptés (PRIORITAIRES)")
        if signes_int:
            blocs.append("- Signes interceptés : " + ", ".join(signes_int))
        if maisons_int:
            for signe in sorted(maisons_int.keys()):
                blocs.append(f"- {signe} intercepté en {maisons_int[signe]}")
        blocs.append("")  # ligne vide pour lisibilité
...
def construire_selection_point_astral(data: Dict[str, Any], max_orbe: float = 5.0) -> str:
    ...
    data_theme = data  # 🔑 alias pour compatibilité avec le reste
...
def retrogradation_lente_pertinente(
    theme: dict,
    planete: str,
    maisons_cibles: set[int] | None = None,
    planetes_personnelles: set[str] | None = None,
    aspects_autorises: set[str] | None = None,
    orbe_max: float = 5.0,
) -> bool:
    """
    Détermine si la rétrogradation d'Uranus, Neptune ou Pluton
    est suffisamment personnelle pour être interprétée.

    Elle est retenue si la planète :
    - gouverne l'une des maisons ciblées ;
    - ou forme un aspect majeur suffisamment serré
      avec une planète personnelle.

    Les autres planètes rétrogrades sont toujours retenues.
    """
    lentes = {"Uranus", "Neptune", "Pluton"}

    if planete not in lentes:
        return True

    placement = (
        (theme.get("planetes") or {}).get(planete, {})
        or {}
    )

    if not (placement.get("retrograde") or placement.get("rx")):
        return False

    if maisons_cibles is None:
        maisons_cibles = {1, 4, 7, 10}

    if planetes_personnelles is None:
    ...
    # 1. La planète gouverne-t-elle une maison angulaire ciblée ?
    rulers_map = theme.get("house_rulers_map") or {}

    maisons_gouvernees = {
        int(maison)
        for maison in (rulers_map.get(planete) or [])
        if str(maison).isdigit()
    }

    if maisons_gouvernees & set(maisons_cibles):
        return True

    # 2. Aspect structurant avec une planète personnelle
    for aspect in theme.get("aspects", []) or []:
        p1 = aspect.get("planete1") or aspect.get("p1")
        p2 = aspect.get("planete2") or aspect.get("p2")
        nature = aspect.get("aspect")
        ...
        if autre not in planetes_personnelles:
            continue

        if nature not in aspects_autorises:
            continue

        try:
            orbe = float(
                str(aspect.get("orbe", 99)).replace(",", ".")
            )
        except (TypeError, ValueError):
            continue
